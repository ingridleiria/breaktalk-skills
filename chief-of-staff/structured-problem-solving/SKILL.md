...
Two failure modes bracket everything below. Answering fast without structure produces a confident opinion nobody can check. Structuring exhaustively without committing to an answer produces six weeks of analysis and a summary. The method exists to avoid both.
...
Also establish the constraints already fixed: a budget cap, a hiring freeze, a commitment made to a customer, a preference the chief executive has expressed in a way nobody will contradict. Analysis that ignores a fixed constraint is theatre, and doing it anyway costs credibility that is needed later.

## Step 2: State the problem as one question
...
Test the statement: is it specific, is the answer measurable, is it actually actionable by the person deciding, and is it bounded in time. A problem statement failing any of these will produce work that cannot conclude.

## Step 3: Structure before you gather
...
Say out loud how confident you are and on what basis. Hypothesis, working answer, or conclusion are different states and should not be reported in the same tone.

## Step 5: Prioritise by what would change the answer
...
## Step 6: Ghost the exhibit before doing the analysis
...
## Step 7: Analyse with proportion
...
## Step 8: Synthesise, do not summarise
...
Build the answer as a pyramid. One governing statement at the top, which is the answer to the question, supported by a small number of arguments that do not overlap and together are sufficient, each supported by evidence. Read upward and each level should answer "why should I believe that". Read downward and each level should answer "so what". This structure is associated with Barbara Minto and it is the most useful thing in the tradition.
...
## Step 10: Attack it before someone else does
...
Choose by audience, not by habit. A decision memo when a choice must be made, for which `decision-memo` has the full shape. An options comparison as a table with options as columns and criteria as rows, every cell carrying evidence rather than an adjective, ending in a recommendation and never in "it depends". A one-page executive summary. Board or investor material through `board-deck`. A briefing for someone walking into a meeting through `executive-briefing`.

## Quality bar

- The decision, the decider, and what would change their mind are established before any analysis.
- The problem is one closed, time-bound question, and the complication makes it inevitable.
- The structure exists in writing before data is gathered, and its branches do not overlap or leave gaps.
- A day-one answer was written and then attacked, not defended.
- Every analysis done would have changed the recommendation if it had come out differently.
- Important numbers are triangulated by a second method and sanity checked against something known.
- The document is a synthesis, not a summary, and the titles alone carry the argument.
- The strongest objection is answered inside the document.
