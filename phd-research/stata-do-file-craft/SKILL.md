...
**The Stata version the work will be run under, and the version any coauthor has.** A `version` statement pins behaviour that has genuinely changed between releases, and the lowest version in the team is the one that constrains the code. Missing: write `version` for the oldest release you know is in use, and note the assumption at the top of the master file.
...
**Result.** The master ran in 22 minutes from raw files to eleven tables and six figures. The published coefficient was 0.062 with firm clustering and 0.059 with state clustering across 38 clusters, reported honestly in the response as a clustering effect with a note that 38 clusters sits at the edge of where the asymptotics are comfortable. The 46 observations were explained in two sentences. The rebuild took four and a half days against an estimate of two.
