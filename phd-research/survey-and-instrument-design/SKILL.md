...
2. **Search for an existing validated instrument, and prefer it even when it is imperfect.** Three reasons, and each is worth more than the elegance of your own wording: its measurement properties are already known, its results are comparable to other work, and referees do not have to take your word for it. Where to look, by name rather than in general: PsycTESTS and the Health and Psychosocial Instruments database index measures directly; the PhenX Toolkit publishes standard protocols for health and behavioural research; the PROMIS item banks cover health outcomes with published item parameters; ERIC indexes education instruments. Large panel studies publish their questionnaires and are the fastest source of an item that has already been fielded at scale: the European Social Survey, the International Social Survey Programme, Understanding Society, SHARE, the Demographic and Health Surveys, and in the United States the Panel Study of Income Dynamics, the Health and Retirement Study and NHANES. The background questionnaires for PISA and TIMSS are the equivalent in education. Cite the original validation paper for whatever you take, not the paper you found it in. The rule for choosing between two candidates: prefer the one used in your population over the one with better psychometrics in a different population, because population fit is harder to argue back than a reliability coefficient.
...
Report reliability as a named coefficient, never as the bare word. Cronbach's alpha is the default in most fields and assumes items are essentially tau-equivalent, which they usually are not; a congeneric reliability coefficient estimated from the scale's factor model, usually reported as McDonald's omega, is the better figure and is increasingly what psychology reviewers ask for. The working conventions: 0.70 and above is treated as acceptable for research on group means, 0.80 and above when groups are being compared, and 0.90 and above where a score will inform a decision about an individual. Above roughly 0.95 usually signals redundant items rather than an excellent scale. For a scale of binary items the equivalent is KR-20. For stability over time it is a test-retest intraclass correlation, conventionally 0.70 or above across an interval short enough that the construct should not have moved. For anything a person codes, it is an intercoder agreement statistic, Cohen's kappa or Krippendorff's alpha, with 0.70 as a floor and 0.80 as comfortable. One caution that matters at pilot scale: alpha rises with the number of items, so a three-item scale can fail a threshold that a ten-item scale of the same item quality passes. Report the coefficient with the number of items and the mean inter-item correlation, which for most scales should sit between about 0.15 and 0.50. The judgement call is whether a pilot problem needs a redesign or a tweak, and the rule is that anything found in cognitive interviewing about meaning requires a rewrite, while anything found in the live pilot about distribution may be acceptable if the analysis can tolerate it.
...
- The live pilot was analysed as data, with completion time, dropout, item nonresponse, distributions and scale reliability reported, the reliability coefficient named and the number of items given alongside it.
...
The worked example is a fifteen-minute self-completion teacher survey on a mixed device population. The logic holds wherever you field it; the budgets, modes and conventions do not.

- **The length budget.** Fifteen minutes assumes an unpaid self-completion survey. A paid panel or a lab session tolerates thirty to forty, a telephone interview loses people at ten, a clinic waiting room gives you five. Set it from your mode and incentive, then cut whole constructs.
- **The pilot size.** Thirty to a hundred shows distributions and dropout. It is not enough for a factor structure: a confirmatory factor analysis or a measurement invariance test across groups needs a few hundred, in Mplus, `lavaan` or Amos, and is a separate stage rather than something the pilot delivers.
- **The reliability convention.** The thresholds above are the general ones. Clinical and high-stakes assessment work to 0.90. Take the number your field's reviewers use and say which coefficient it belongs to.
- **The measure archives.** Those named above skew to health, psychology and large social surveys. In education add the national assessment frameworks; in sociology the ISSP module archive; in economics the questionnaire appendices of the panel you are extending.
- **What not to change.** Cognitive interviews before any live pilot, a blueprint that reads in both directions, and a validated instrument preferred over an invented one.
